...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const register = async (formData: any) => {
    try {
      ...
      if (error.response?.data?.errors && error.response.data.errors.length > 0) {
        const validationError = error.response.data.errors[0].msg || error.response.data.errors[0].message;
  ...
  const login = async (email: string, password: string, userType: 'student' | 'educator') => {
    try {
